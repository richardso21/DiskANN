...
class DiskannIndexBuilder:
    ...
    def __init__(
        ...
        self.index_save_path = os.path.join(self.save_dir, f"e5_diskann_{self.search_mem_max}")
    ...
    def encode_all(self):
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size), desc="Inference Embeddings:"):
            ...
            if "T5" in type(self.encoder).__name__ or (self.gpu_num > 1 and hasattr(self.encoder, 'module') and "T5" in type(self.encoder.module).__name__):
                ...
                embeddings = pooling(
                    output.pooler_output, output.last_hidden_state, inputs["attention_mask"], self.pooling_method # type: ignore
                )
